...
int main(int argc, char* argv[])
{
  ...
  // Forward FFT
  imProcessFFT(image, fft_image);	// Fix: imProcessFFTW was renamed to imProcessFFT
  ...
  // Inverse FFT
  imProcessIFFT(fft_image, ifft_image);	// Fix: imProcessIFFTW was renamed to imProcessIFFT
  ...
  if (image->data_type != IM_CFLOAT)
  {
    ...
    imProcessConvertDataType(ifft_image, image, IM_CPX_REAL, IM_GAMMA_LINEAR, 1, IM_CAST_MINMAX); // Fix: use imProcessConvertDataType instead of imConvertDataType
  }
